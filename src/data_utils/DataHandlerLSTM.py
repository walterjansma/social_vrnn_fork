...
class DataHandlerLSTM():
    ...
    def processData(self, **kwargs):
        ...
        """
        data_pickle = self.home + self.args.data_path + self.args.scenario + "/data" + str(self.args.prediction_horizon) + "_" + str(
    ...
    def load_map(self,**kwargs):
        ...
        print(os.path.isfile(self.home + self.data_path+self.args.scenario + '/map.json'))


        if not os.path.isfile(self.home + self.data_path+self.args.scenario + 'map.json'): # was /map.npy
            print("Creating map from png ...")
            print(self.args.data_path+self.args.scenario)
            sup.create_map_from_png(data_path=self.args.data_path+self.args.scenario,**kwargs)

        with open(os.path.join(self.home + self.data_path+self.args.scenario, 'map.json')) as json_file:
            map_data = json.load(json_file)

        grid_map = pl.imread(self.home + self.data_path + 'map.png')
        grid_map = np.ceil(grid_map[:,:,0])

        #map_data = np.load(os.path.join(home + self.data_path+self.args.scenario, 'map.png'), encoding='latin1', allow_pickle=True)
        self.agent_container.occupancy_grid.gridmap = grid_map  # occupancy values of cells
        self.agent_container.occupancy_grid.resolution = map_data['resolution']  # map resolution in [m / cell]
        self.agent_container.occupancy_grid.map_size = map_data['map_size']  # map size in [m]
        self.agent_container.occupancy_grid.center = map_data['map_center']
    ...
    def _process_simulation_data_(self, **kwargs):
        ...
        pedestrian_data = np.genfromtxt(os.path.join(self.home + self.data_path+self.args.scenario, 'datafile.csv'), delimiter=",")[1:, :]
        ...
        for sample_idx in range(pedestrian_data.shape[0]):
            if pedestrian_data[sample_idx, 0] != -1:
                ...
                vel[:,0:2] = np.true_divide(pedestrian_data[sample_idx, 6:8], np.array([self.norm_const_vx,self. norm_const_vy]))
                #print(pedestrian_data[sample_idx, :])
                goal = np.true_divide(pedestrian_data[sample_idx, 9:], np.array([self.norm_const_x, self.norm_const_y]))
...
#           traj.smoothenTrajectory(dt=self.dt)
    ...
    def saveTrajectoryData(self, save_path):
        ...
        if not os.path.isdir(self.home + self.args.data_path + self.args.scenario):
            os.makedirs(self.home + self.args.data_path + self.args.scenario)
