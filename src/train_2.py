...
with tf.Session(config=config) as sess:
    ...
    for step in range(initial_step, args.total_training_steps):
        ...
        dict = {"batch_x": batch_x, # (batch_size, tbpl, rnn_state_size)
                "batch_vel": batch_vel, # (batch_size, tbpl, rnn_state_ped_size)
                "batch_pos": batch_pos, # (batch_size, tbpl, rnn_state_ped_size)
                "batch_goal": batch_goal,
                  "batch_grid": batch_grid, # (batch_size, tbpl, 60, 60) (60,60) = size of submap
                  "batch_ped_grid": batch_ped_grid, # (batch_size, tbpl, pedestrian_vec_dim)
                  "step": step,
                  "batch_y": batch_y, # (batch_size, tbpl, 2*pred_horizon)
                  "batch_pos_target": batch_pos_target,
                "batch_div": batch_y, # batch_y
                "other_agents_pos": other_agents_pos # (batch_size, tbpl, n_other_agents, ?)
                  }
        
        print(dict)
        print(dict['batch_grid'].shape)
        print(dict['batch_ped_grid'].shape)
        print(dict['batch_x'].shape)
        print(dict['batch_y'].shape)
        print(dict['batch_vel'].shape)
        print(dict['batch_pos'].shape)
        print(np.shape(dict['other_agents_pos']))
